        ...
        <v-card elevation="3" color="white" :class="['ma-4', selectedClass, 'text-left']" v-bind="props" width="250">
                ...
                <v-img height="200" :src="cardEvent.img" cover class="text-white">
                    ...
                    <div v-if="isHovering"
                        ...
                        style="height: 100%;">
                        <v-btn v-if="loggedIn" class="w-75 ma-5 bg-white" @click="rsvp(cardEvent.id, !isSelected)"
                            v-on:click="toggle"><span v-if="isSelected">Cancel</span>
                            RSVP</v-btn>
                        <v-btn class="w-75" :href="`/events/id/${cardEvent.id}`">More Info</v-btn>
                    </div>
